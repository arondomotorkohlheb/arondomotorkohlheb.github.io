...
def nodei_opt(Ai, Bi, x0i, T, u_max, lambda_plus, lambda_minus, terminal_weight=None, input_weight=None):
    n, m = Bi.shape
    ...
    if terminal_weight is None:
        terminal_diag = np.ones(n)
        terminal_diag[-1] = 10.0
        terminal_weight = np.diag(terminal_diag)
    else:
        terminal_weight = np.asarray(terminal_weight)
        if terminal_weight.ndim == 1:
            terminal_weight = np.diag(terminal_weight)

    if input_weight is None:
        input_diag = np.ones(m)
        if m > 1:
            input_diag[-1] = 0.25
        input_weight = np.diag(input_diag)
    else:
        input_weight = np.asarray(input_weight)
        if input_weight.ndim == 1:
            input_weight = np.diag(input_weight)

    u_blocks = [ui[i * m:(i + 1) * m] for i in range(T)]
    input_cost = cp.sum([cp.quad_form(block, input_weight) for block in u_blocks])
    Ji = cp.sum_squares(bar_Ai @ x0i + bar_Bi @ ui) + input_cost
    obji = Ji + cp.quad_form(xiT, terminal_weight) + lambda_plus.T @ xiT - lambda_minus.T @ xiT

    u_bound = np.asarray(u_max).reshape(-1)
    if u_bound.size == 1:
        u_bound = np.repeat(u_bound.item(), m)
    u_bound = np.tile(u_bound, T).reshape(-1, 1)
    constraintsi = [cp.abs(ui) <= u_bound]
