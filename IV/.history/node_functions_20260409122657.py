...
def nodei_opt(Ai, Bi, x0i, T, u_max, lambda_plus, lambda_minus, terminal_weight=1e-2):
    n, m = Bi.shape
    ...
    obji = Ji + terminal_weight * cp.sum_squares(xiT) + lambda_plus.T @ xiT - lambda_minus.T @ xiT
...
def update_lambda(lambdas, alpha, A_list, B_list, x0_list, T, u_max):
    ...
    # Normalize the residual so a weakly actuated state does not dominate the dual step.
    state_scale = np.maximum(1.0, np.max(np.abs(v), axis=0, keepdims=True))
    lambdas_new = lambdas + alpha * (v / state_scale)
...
def update_lambda_nesterov(lambdas, y, alpha, beta, A_list, B_list, x0_list, T, u_max):
    ...
    v = np.vstack([(xT[i] - xT[i + 1]).reshape(1, -1) for i in range(n_nodes - 1)])
    state_scale = np.maximum(1.0, np.max(np.abs(v), axis=0, keepdims=True))
    lambdas_new = y + alpha * (v / state_scale)
