...
def ADMM_update(rho, lambda_array, z, xf_array, A_list_local, B_list_local, x0_list_local, T_local, u_max_local):
    n_nodes = len(A_list_local)
    for i in range(n_nodes):
        xf_array[i, :, :] = nodei_xfi(
            A_list_local[i],
            B_list_local[i],
            x0_list_local[i],
            T_local,
...
def ADMM(rho=100, max_iter=20, A_list_local=None, B_list_local=None, x0_list_local=None, T_local=None, u_max_local=None):
    if A_list_local is None:
        A_list_local = A_list
    if B_list_local is None:
        B_list_local = B_list
    if x0_list_local is None:
        x0_list_local = x0_list
    if T_local is None:
        T_local = T
    if u_max_local is None:
        u_max_local = u_max

    n_nodes = len(A_list_local)
    n = A_list_local[0].shape[0]
    x0_stack = np.array([np.asarray(x0).reshape(n, 1) for x0 in x0_list_local])
    z = np.mean(x0_stack, axis=0)
    lambda_array = np.ones((n_nodes, n, 1))
    xf_array = x0_stack.copy()
    xf_array_hist = [xf_array.copy()]
    for iter in range(max_iter):
        try:
            xf_array, z, lambda_array = ADMM_update(
                rho,
                lambda_array,
                z,
                xf_array,
...
def error_vs_rho():
    ...
    for rho in np.linspace(58, 68, 5):
...
if __name__ == "__main__":
    _, xf_array_hist = ADMM(rho=64, max_iter=60, x0_list_local=x0_list)
