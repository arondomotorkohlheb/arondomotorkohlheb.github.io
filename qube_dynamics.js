...
class Qube {
    ...
    // Update reference state based on configured tracking mode.
    referenceTracking() {
        if (this.tracking === "sinusoidal") {
            const f = 4;
            this.xref = [0, 0, 0.4 * Math.sin(f * this.t), 0];
        } else if (this.tracking === "square") {
            const f = 8 / Math.PI;
            this.xref = [0, 0, 0.4 * Math.sign(Math.sin(f * this.t)), 0];
        } else {
            this.xref = [0, 0, 0, 0];
        }
    }
